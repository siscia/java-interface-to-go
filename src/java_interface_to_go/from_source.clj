(ns java-interface-to-go.from-source
...
(defn get-extendee [str]
  (when-let [extension  (re-matches #"(.|\n)* extends ([^\{]*)\{(.|\n)*" str)]
    (->
     extension
     (nth 2)
     (clojure.string/split #","))))

(defn get-name-interface [str]
  (when-let [name (is-interface? str)]
    (nth name 2)))

(defn get-methods [str]
  (re-seq #"\{\n+|\s*(\w+) (\w+)\((.*?)\)\s*\}*;" str))

(defn type-params [params]
  (let [coma (clojure.string/split params #",")
        types (mapv #(-> %
                         clojure.string/trim
                         (clojure.string/split #" ")) coma)]
    (clojure.string/join ", "(mapv first types))))
